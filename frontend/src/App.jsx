...
const DEFAULT_HERO_SLIDES = [
  { kicker: 'The daily command center', title: 'Make every sale feel organised.', copy: 'Invoices, stock, GST, contacts, and follow-ups move together so your team can focus on customers.', stat: '01 / 03', accent: 'Billing that keeps pace', imageUrl: '', buttonLabel: 'Open workspace', buttonLink: '/login' },
  { kicker: 'From first call to final payment', title: 'Turn busy work into a clear pipeline.', copy: 'Give callers a focused workspace and give admins the visibility to act before good leads go cold.', stat: '02 / 03', accent: 'People-first operations', imageUrl: '', buttonLabel: 'Create account', buttonLink: '/register' },
...
function App() {
  ...
  useEffect(() => {
    api.get('/users/public-settings').then((response) => {
      ...
      document.documentElement.style.setProperty('--app-font', settings.fontFamily === 'DM Sans' ? 'DM Sans, sans-serif' : settings.fontFamily === 'Plus Jakarta Sans' ? 'Plus Jakarta Sans, sans-serif' : 'Manrope, sans-serif');
      document.documentElement.style.setProperty('--radius-lg', `${Number(settings.cornerRadius || 20)}px`);
      document.documentElement.style.setProperty('--radius-md', `${Math.max(8, Number(settings.cornerRadius || 20) - 6)}px`);
...
function PublicApp({ setUser }) {
  ...
  const stats = Array.isArray(settings.publicStats) && settings.publicStats.length ? settings.publicStats : DEFAULT_STATS;
  const copy = PUBLIC_TRANSLATIONS[settings.language] || PUBLIC_TRANSLATIONS.en;
  ...
  return (
    <div id="top" className={`public-site ${authRoute ? 'auth-route' : ''}`}>
      <header className="public-nav"><a className="public-brand" href="#top"><span className="public-brand-mark">SG</span><span>{settings.siteTitle}</span></a><nav><a href="#about">{copy.about}</a><a href="#contact">{copy.contact}</a><Link className="btn primary" to="/login">{copy.login}</Link></nav></header>
      ...
      <section className="public-section public-feature-band" id="capabilities"><div className="public-section-heading"><div><p className="eyebrow">One workspace, many moving parts</p><h2>Run the whole solar business from a calmer control room.</h2></div><p>SGSE brings the customer conversation, project costing, inventory, billing, finance, and people operations into one place so work keeps moving after the first call.</p></div><div className="public-feature-grid public-feature-grid-rich"><div><b className="public-feature-icon">01</b><strong>Quote with confidence</strong><span>Create manual solar designs, calculate payback, apply subsidy scenarios, and download a polished PDF proposal.</span><a href="#contact">Explore quotations <span aria-hidden="true">→</span></a></div><div><b className="public-feature-icon">02</b><strong>Keep stock in step</strong><span>Track items, setup kits, GST rates, stock movement, and invoices without copying details between tools.</span><a href="#contact">See the operations view <span aria-hidden="true">→</span></a></div><div><b className="public-feature-icon">03</b><strong>Stay close to customers</strong><span>Move leads through follow-ups, documents, loans, installation, subsidy, and service with a clear project trail.</span><a href="#contact">Build your pipeline <span aria-hidden="true">→</span></a></div></div></section>

      <section className="public-section public-control-band"><div className="public-control-copy"><p className="eyebrow">A useful view at every stage</p><h2>From a new lead to a completed installation.</h2><p>Give each person the right amount of visibility. Callers get a focused queue, admins get control, and the wider team gets a shared source of truth.</p><div className="public-check-list"><span><i>✓</i> Contacts and follow-up history</span><span><i>✓</i> 3D EPC quotation builder</span><span><i>✓</i> GST billing and PDF invoices</span><span><i>✓</i> Reports, payroll, and accounting</span></div><Link className="btn primary" to="/register">Start your workspace</Link></div><div className="public-control-panel"><div className="control-panel-top"><span><i /><i /><i /></span><small>SGSE / Operations overview</small><b>Live</b></div><div className="control-panel-title"><div><small>Today&apos;s project flow</small><strong>Everything in motion</strong></div><span>Updated now</span></div><div className="control-panel-grid"><div><small>Open quotations</small><strong>18</strong><em className="up">+12%</em></div><div><small>Installation queue</small><strong>07</strong><em>Next 14 days</em></div><div><small>Stock health</small><strong>94%</strong><em className="up">On track</em></div></div><div className="control-panel-list"><div><span className="panel-status green" /><b>New 5 kW proposal</b><small>Ready for review</small></div><div><span className="panel-status gold" /><b>Customer documents</b><small>3 items pending</small></div><div><span className="panel-status blue" /><b>Invoice collection</b><small>₹1.24L this month</small></div></div></div></section>

      <section className="public-section public-testimonial"><div><p className="eyebrow">Made for the everyday handoffs</p><h2>Less chasing. More finishing.</h2><p>When sales, field work, finance, and support share the same context, customers feel the difference.</p></div><blockquote><span>&ldquo;</span><p>We stopped rebuilding the same project details in three different places. The team can see what is happening and act on it.</p><footer><strong>Solar operations team</strong><small>Built around the work that happens every day</small></footer></blockquote></section>

      {settings.showPublicAbout !== false && <section className="public-section public-about" id="about"><div><p className="eyebrow">{copy.aboutEyebrow}</p><h2>{settings.aboutTitle}</h2></div><p>{settings.aboutText}</p><div className="public-feature-grid"><div><strong>Billing</strong><span>GST-ready invoices and clean payment tracking.</span></div><div><strong>Operations</strong><span>Stock, contacts, callers, and payroll in one view.</span></div><div><strong>Finance</strong><span>Reports, accounts, and planning tools for every day.</span></div></div></section>}

      {settings.showPublicContact !== false && <section className="public-section public-contact" id="contact"><div><p className="eyebrow">{copy.contactEyebrow}</p><h2>{settings.contactCta}</h2><p>{copy.contactText}</p></div><div className="public-contact-details"><a href={`tel:${settings.publicPhone || ''}`}>{settings.publicPhone || 'Phone available after sign-in'}</a><a href={`mailto:${settings.publicEmail || ''}`}>{settings.publicEmail || 'Email available after sign-in'}</a><Link className="btn primary" to="/login">{copy.open}</Link></div></section>}

      <div className="auth-panel">
        <div className="auth-ambient" />
        {authRoute && <div className="auth-welcome-panel"><span className="auth-welcome-kicker">{settings.siteTagline}</span><h2>{settings.siteTitle}</h2><p>{settings.siteDescription}</p><div className="auth-welcome-metrics"><span><strong>01</strong><small>One connected workspace</small></span><span><strong>24/7</strong><small>Visibility for every team</small></span><span><strong>GST</strong><small>Ready for Indian billing</small></span></div><div className="auth-welcome-orbit" aria-hidden="true"><i /><i /><i /><b>SG</b></div></div>}
        <Routes>
      ...
      {!authRoute && <footer className="public-footer"><div className="public-footer-main"><div className="public-footer-brand"><a className="public-brand" href="#top"><span className="public-brand-mark">SG</span><span>{settings.siteTitle}</span></a><p>{settings.siteDescription}</p><span className="public-footer-badge"><i /> Built for calmer solar operations</span></div><div className="public-footer-column"><strong>Workspace</strong><a href="#capabilities">Capabilities</a><Link to="/login">Open workspace</Link><Link to="/register">Create account</Link><a href="#about">About SGSE</a></div><div className="public-footer-column"><strong>Operations</strong><a href="#capabilities">3D quotations</a><a href="#capabilities">Billing and GST</a><a href="#capabilities">Stock control</a><a href="#capabilities">Customer pipeline</a></div><div className="public-footer-column"><strong>Connect</strong><a href={`tel:${settings.publicPhone || ''}`}>{settings.publicPhone || 'Phone after sign-in'}</a><a href={`mailto:${settings.publicEmail || ''}`}>{settings.publicEmail || 'Email after sign-in'}</a><a href="#contact">Talk to the team</a><Link className="footer-cta" to="/register">Get started <span aria-hidden="true">→</span></Link></div></div><div className="public-footer-bottom"><span>© {new Date().getFullYear()} {settings.siteTitle}. Built for work that keeps moving.</span><span className="public-footer-status"><i /> Systems ready</span><a href="#top">Back to top ↑</a></div></footer>}
    </div>
  );
}
...
function EmiCalculatorPage() {
  ...
  const affordabilityRatio = Number(form.monthlyIncome) > 0 ? ((result.emi + result.extraMonthlyPayment) / Number(form.monthlyIncome)) * 100 : 0;
  ...
  const compactMoney = (value) => `₹${Number(value || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`;
  const tenureLabel = (months) => `${months} months · ${(Number(months || 0) / 12).toFixed(1)} years`;
  const principalPercent = result.cost ? Math.min(100, (result.principal / result.cost) * 100) : 0;
  const validationMessage = Number(form.downPayment) > Number(form.loanAmount) ? 'Down payment cannot exceed the asset amount.' : Number(form.extraMonthlyPayment) >= result.emi && result.principal > 0 ? 'Extra payment is large enough to pay this loan down quickly.' : '';
  const update = (field, value) => setForm((current) => ({ ...current, [field]: value }));
  const applyPreset = (preset) => setForm((current) => ({ ...current, downPayment: '0', extraMonthlyPayment: '0', monthlyIncome: current.monthlyIncome || '50000', ...preset }));
  const resetCalculator = () => setForm({ loanAmount: '200000', downPayment: '0', annualRate: '10.5', tenureMonths: '60', processingFeeRate: '1', extraMonthlyPayment: '0', monthlyIncome: '50000' });

  const downloadSchedule = () => {
    const rows = [['Month', 'Opening balance', 'Payment', 'Principal', 'Interest', 'Closing balance'], ...result.schedule.map((row) => [row.month, row.openingBalance, row.emi, row.principalPaid, row.interest, row.closingBalance])];
  ...
  return <div className="emi-page">
    <div className="page-header"><p className="eyebrow">Finance toolkit</p><h3>EMI calculator</h3><p className="muted">Build a practical repayment plan, understand the real cost, and choose a tenure that fits your monthly cash flow.</p></div>
    <div className="emi-layout">
      <section className="panel emi-input-panel">
        <div className="panel-header"><div><h4>Set up your plan</h4><p className="muted">Use a preset, then fine-tune the numbers for your offer.</p></div><span className="emi-badge">Reducing balance</span></div>
        <div className="emi-quick-actions"><label>Start with a preset<select value="" onChange={(event) => { const preset = event.target.value; if (preset === 'solar') applyPreset({ loanAmount: '500000', annualRate: '9.5', tenureMonths: '84', processingFeeRate: '1' }); if (preset === 'vehicle') applyPreset({ loanAmount: '800000', annualRate: '10.5', tenureMonths: '60', processingFeeRate: '1.5' }); if (preset === 'equipment') applyPreset({ loanAmount: '300000', annualRate: '11.5', tenureMonths: '48', processingFeeRate: '1' }); }}><option value="">Choose a common plan</option><option value="solar">Solar project</option><option value="vehicle">Vehicle finance</option><option value="equipment">Equipment finance</option></select></label><button className="btn outline" type="button" onClick={resetCalculator}>Reset</button></div>
        <div className="form-grid">
          <label>Asset or loan amount<small>Purchase price before down payment</small><input type="number" min="0" step="1000" value={form.loanAmount} onChange={(event) => update('loanAmount', event.target.value)} /></label>
          <label>Down payment<small>Pay upfront to reduce the loan</small><input type="number" min="0" step="1000" value={form.downPayment} onChange={(event) => update('downPayment', event.target.value)} /></label>
          <label>Annual interest rate (%)<small>Nominal reducing-balance rate</small><input type="number" min="0" step="0.01" value={form.annualRate} onChange={(event) => update('annualRate', event.target.value)} /></label>
          <label>Tenure (months)<small>{tenureLabel(form.tenureMonths)}</small><input type="number" min="1" max="360" step="1" value={form.tenureMonths} onChange={(event) => update('tenureMonths', event.target.value)} /><input className="emi-range" aria-label="Tenure slider" type="range" min="6" max="360" step="6" value={Math.min(360, Math.max(6, Number(form.tenureMonths) || 6))} onChange={(event) => update('tenureMonths', event.target.value)} /></label>
          <label>Processing fee (%)<small>Charged on the financed principal</small><input type="number" min="0" step="0.01" value={form.processingFeeRate} onChange={(event) => update('processingFeeRate', event.target.value)} /></label>
          <label>Extra monthly payment<small>Optional prepayment above EMI</small><input type="number" min="0" step="500" value={form.extraMonthlyPayment} onChange={(event) => update('extraMonthlyPayment', event.target.value)} /></label>
          <label>Monthly take-home income<small>Used only for affordability guidance</small><input type="number" min="0" step="1000" value={form.monthlyIncome} onChange={(event) => update('monthlyIncome', event.target.value)} /></label>
        </div>
        {validationMessage && <p className="emi-inline-warning">{validationMessage}</p>}
        <div className={`emi-affordability ${affordabilityRatio > 45 ? 'risk' : affordabilityRatio > 30 ? 'watch' : ''}`}><strong>{affordabilityLabel}</strong><span>{affordabilityRatio ? `${affordabilityRatio.toFixed(1)}% of monthly income including extra payment` : 'Add income to see affordability'}</span></div>
        <p className="muted emi-note">Guidance only: lenders may apply taxes, insurance, rounding, foreclosure rules, or other charges separately.</p>
      </section>
      <section className="emi-result-panel">
        <div className="emi-highlight"><span>Estimated monthly outflow</span><strong>{money(result.emi + result.extraMonthlyPayment)}</strong><small>Base EMI {money(result.emi)}{result.extraMonthlyPayment ? ` + ${money(result.extraMonthlyPayment)} extra` : ''}</small></div>
        <div className="emi-metrics"><div><span>Financed principal</span><strong>{money(result.principal)}</strong></div><div><span>Total interest</span><strong>{money(result.totalInterest)}</strong></div><div><span>Total repayment</span><strong>{money(result.totalPayment)}</strong></div><div><span>Payoff period</span><strong>{tenureLabel(result.schedule.length)}</strong></div></div>
        <div className="emi-result-progress"><div><span>Financed</span><strong>{principalPercent.toFixed(0)}%</strong></div><div className="emi-progress-track"><span style={{ width: `${principalPercent}%` }} /></div><small>{result.downPayment ? `${compactMoney(result.downPayment)} paid upfront` : 'No down payment included'}</small></div>
    ...
    <section className="panel emi-compare"><div className="panel-header"><div><h4>Choose your repayment pace</h4><p className="muted">A shorter tenure usually costs more each month but reduces total interest.</p></div><select value={compareTenure} onChange={(event) => setCompareTenure(event.target.value)}><option value="24">24 months</option><option value="36">36 months</option><option value="48">48 months</option><option value="60">60 months</option><option value="84">84 months</option></select></div><div className="emi-compare-grid"><div><span>Current plan</span><strong>{money(result.emi)}</strong><small>{tenureLabel(result.schedule.length)} · {money(result.totalInterest)} interest</small></div><div className="compare-arrow">→</div><div><span>Compared plan</span><strong>{money(compareResult.emi)}</strong><small>{tenureLabel(compareResult.schedule.length)} · {money(compareResult.totalInterest)} interest</small></div><div className="emi-saving"><span>{compareResult.totalInterest <= result.totalInterest ? 'Potential saving' : 'Extra interest'}</span><strong>{money(Math.abs(result.totalInterest - compareResult.totalInterest))}</strong><small>{compareResult.emi > result.emi ? `${money(compareResult.emi - result.emi)} more per month` : `${money(result.emi - compareResult.emi)} less per month`}</small></div></div></section>
...
function WebAppSettingsPage() {
  ...
  return <div className="settings-page"><div className="page-header"><p className="eyebrow">Control center</p><h3>Web app settings</h3><p className="muted">Control the workspace, public website, navigation, and visual identity from one place.</p></div><form className="panel settings-panel" onSubmit={save}>
    <div className="settings-manual"><div><p className="eyebrow small">Workspace manual</p><h4>Set it once, make every screen feel intentional.</h4><p>Use this control center in order: define your public identity, shape the homepage, choose the visual system, then tune operations and accessibility.</p></div><div className="settings-manual-grid"><span><b>01</b><strong>Brand</strong><small>Name, language, contact, and public copy</small></span><span><b>02</b><strong>Present</strong><small>Hero slides, proof points, and themes</small></span><span><b>03</b><strong>Operate</strong><small>Modules, defaults, and reminders</small></span><span><b>04</b><strong>Include</strong><small>Font, size, and readable navigation</small></span></div></div>
...
function EmployeesPage() {
  ...
  const saveEmployee = async (event) => {
    ...
    try {
      const payload = {
        ...
        homeRentAllowance: Number(form.homeRentAllowance || 0),
  ...
  const downloadPayslipPdf = async () => {
    ...
    const drawTable = (rows, widths, rowHeight = 7) => {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        row.forEach((cell, cellIndex) => {
          ...
          doc.setFontSize(rowIndex === 0 ? 7.5 : 7.2);
          doc.text(String(cell), x + (cellIndex % 2 ? width - 2 : 2), y + 4.6, { align: cellIndex % 2 ? 'right' : 'left' });
          x += width;
        });
        y += rowHeight;
      });
    ...
    drawTable([
      ...
      ['Designation', valueOrDash(employee.role), 'Paid Days', String(payslip.payableDays || 0)],
      ['Department', '—', 'Bank Name', valueOrDash(personalDetails.bankName)],
      ['Date of Joining', valueOrDash(employee.joiningDate), 'Bank A/c No.', valueOrDash(personalDetails.accountNumber)],
      ['Gross Salary', money(employee.monthlySalary), 'UAN', '—']
    ...
    drawTable([
      ['Earnings', '', 'Deductions', ''],
      ['Basic Salary', money(payslip.basicSalary), 'EPF', '—'],
      ['House Rent Allowances', money(payslip.homeRentAllowance), 'Health Insurance/ESI', '—'],
      ['Conveyance Allowance', money(payslip.fuelAllowance), 'Professional Tax', '—'],
      ['Medical Allowances', money(0), 'Advance', money(payslip.advance)],
      ['Special Allowances', money(Number(employee.otherAllowance || 0) + Number(payslip.bonus || 0) + Number(payslip.incentive || 0)), 'Other Deduction', money(payslip.deduction)],
      ['Gross Salary', money(payslip.gross), 'Total Deduction', money(payslip.totalDeductions)]
    ], [34, 45, 37, contentWidth - 116]);

    y += 2;
    drawTable([['Net Pay', money(payslip.netPay), '', '']], [34, 45, 37, contentWidth - 116]);
    y += 3;
    drawTable([['Total Deduction', money(payslip.totalDeductions), '', '']], [34, 45, 37, contentWidth - 116]);
    y += 18;
    ...
    doc.text('Employee Signature', pageWidth - margin - 21, y + 5, { align: 'center' });
    y += 13;
