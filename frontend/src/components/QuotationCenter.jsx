...
function ThreeDPreview({ form }) {
  ...
  return <div className="quotation-3d-wrap"><div className="quotation-3d-scene" style={{ '--roof-tilt': `${number(form.tilt, 20) - 12}deg`, '--roof-azimuth': `${number(form.azimuth, 180) - 180}deg`, '--panel-opacity': `${Math.max(0.35, 1 - number(form.shading, 5) / 140)}` }}><div className="quotation-roof">{Array.from({ length: count }, (_, index) => <span key={index} style={{ width: `${100 / columns}%`, height: `${100 / rows}%` }} />)}</div><div className="quotation-house" /><div className="quotation-inverter">INVERTER</div></div><div className="quotation-3d-caption">Manual 3D concept · drag-free preview · {form.roofLength || 8}m × {form.roofWidth || 5}m · {count} modules · {form.shading || 5}% shading</div></div>;
}
